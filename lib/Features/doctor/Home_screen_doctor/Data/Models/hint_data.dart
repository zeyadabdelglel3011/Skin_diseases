class HintData {
  final String title , desc , pic;

  HintData({required this.title, required this.desc, required this.pic});


}
List<HintData> hintData = [
  HintData(
    title:"Scan your skin" ,
      desc:"Easily scan your skin using your phone’s camera\nfor an instant analysis of \nvisible skin conditions" ,
      pic: "images/Group1.png",
  ),
  HintData(
    title:"Chat With A Dermatologist \nAnytime To Predict Skin Issues " ,
    desc:"Easily scan your skin using your phone’s camera\nfor an instant analysis of \nvisible skin conditions" ,
    pic: "images/Group 2.png",),
  HintData(
    title:"Take the medication that suits \nyour symptoms" ,
    desc:"Let your doctor determine the right medicine tailored\nto your symptoms for optimal results" ,
    pic: "images/syringe 1.png",),

];

List<HintData> doctorsData  =[
  HintData(
      title: "Dr.Lujain Ahmed",
      desc: "Dr lujain is a dermatologist medical expert specializing in the diagnosis, treatment, and prevention of skin, hair, and nail conditions. They are skilled in managing common concerns like acne, eczema, psoriasis, and skin infections",
      pic: "images/lujy.png",
  ),
  HintData(
    title: "Dr.Ziad Henry",
    desc: "Dr ziad is a dermatologist medical expert specializing in the diagnosis, treatment, and prevention of skin, hair, and nail conditions. They are skilled in managing common concerns like acne, eczema, psoriasis, and skin infections",
    pic: "images/zeyad.png",
  ),
  HintData(
    title: "Dr.Lujain Ahmed",
    desc: "Dr lujain is a dermatologist medical expert specializing in the diagnosis, treatment, and prevention of skin, hair, and nail conditions. They are skilled in managing common concerns like acne, eczema, psoriasis, and skin infections",
    pic: "images/lujy.png",
  ),
  HintData(
    title: "Dr.Ziad Henry",
    desc: "Dr ziad is a dermatologist medical expert specializing in the diagnosis, treatment, and prevention of skin, hair, and nail conditions. They are skilled in managing common concerns like acne, eczema, psoriasis, and skin infections",
    pic: "images/zeyad.png",
  ),
];

List<HintData> patientsData  =[
  HintData(
    title: "Zeyad Abdelglel",
    desc: "Dr lujain is a dermatologist medical expert specializing in the diagnosis, treatment, and prevention of skin, hair, and nail conditions. They are skilled in managing common concerns like acne, eczema, psoriasis, and skin infections",
    pic: "images/zez.png",
  ),
  HintData(
    title: "Zeko",
    desc: "Dr lujain is a dermatologist medical expert specializing in the diagnosis, treatment, and prevention of skin, hair, and nail conditions. They are skilled in managing common concerns like acne, eczema, psoriasis, and skin infections",
    pic: "images/girl.png",
  ),
  HintData(
    title: "Zeyad Abdelglel",
    desc: "Dr lujain is a dermatologist medical expert specializing in the diagnosis, treatment, and prevention of skin, hair, and nail conditions. They are skilled in managing common concerns like acne, eczema, psoriasis, and skin infections",
    pic: "images/zez.png",
  ),
  HintData(
    title: "Zeyad Abdelglel",
    desc: "Dr lujain is a dermatologist medical expert specializing in the diagnosis, treatment, and prevention of skin, hair, and nail conditions. They are skilled in managing common concerns like acne, eczema, psoriasis, and skin infections",
    pic: "images/zez.png",
  ),

];